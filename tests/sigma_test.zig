...
test "schnorr proof of knowledge" {
    ...
    const proof = zkp.sigma.schnorrProve("bsvz-zkp/schnorr/test/v1", G, P, x);
    try std.testing.expect(zkp.sigma.schnorrVerify("bsvz-zkp/schnorr/test/v1", G, P, proof));
}
...
test "schnorr proof fails with wrong secret" {
    ...
    const P = G.mul(x.toBytes()) catch unreachable;
    const proof = zkp.sigma.schnorrProve("bsvz-zkp/schnorr/test/v1", G, P, wrong_x);
    try std.testing.expect(!zkp.sigma.schnorrVerify("bsvz-zkp/schnorr/test/v1", G, P, proof));
}
...
test "schnorr proof fails with wrong label" {
    ...
    const P = G.mul(x.toBytes()) catch unreachable;
    const proof = zkp.sigma.schnorrProve("bsvz-zkp/schnorr/test/v1", G, P, x);
    try std.testing.expect(!zkp.sigma.schnorrVerify("bsvz-zkp/schnorr/test/v2", G, P, proof));
}
...
test "schnorr proof fails with wrong base" {
    ...
    const P = G.mul(x.toBytes()) catch unreachable;
    const proof = zkp.sigma.schnorrProve("bsvz-zkp/schnorr/test/v1", G, P, x);
    try std.testing.expect(!zkp.sigma.schnorrVerify("bsvz-zkp/schnorr/test/v1", zkp.generators.H, P, proof));
}
...
test "cds-or one-of-two known index zero" {
    ...
    const proof = try zkp.sigma.cdsOrProve(std.testing.allocator, "bsvz-zkp/cds-or/test/v1", G, &statements, 0, x0);
    defer zkp.sigma.cdsOrProofDeinit(proof, std.testing.allocator);

    try std.testing.expect(zkp.sigma.cdsOrVerify("bsvz-zkp/cds-or/test/v1", G, &statements, proof));
}
...
test "cds-or one-of-two known index one" {
    ...
    const statements = [_]bsvz.crypto.Point{ P0, P1 };
    const proof = try zkp.sigma.cdsOrProve(std.testing.allocator, "bsvz-zkp/cds-or/test/v1", G, &statements, 1, x1);
    defer zkp.sigma.cdsOrProofDeinit(proof, std.testing.allocator);
...
test "cds-or one-of-three" {
    ...
    const proof = try zkp.sigma.cdsOrProve(std.testing.allocator, "bsvz-zkp/cds-or/test/v1", G, &statements, 2, xs[2]);
    defer zkp.sigma.cdsOrProofDeinit(proof, std.testing.allocator);
...
test "cds-or fails for unknown secret" {
    ...
    const statements = [_]bsvz.crypto.Point{ P0, P1 };
    const proof = try zkp.sigma.cdsOrProve(std.testing.allocator, "bsvz-zkp/cds-or/test/v1", G, &statements, 0, x_unknown);
    defer zkp.sigma.cdsOrProofDeinit(proof, std.testing.allocator);
...
test "cds-or fails for tampered e-values" {
    ...
    const statements = [_]bsvz.crypto.Point{ P0, P1 };
    var proof = try zkp.sigma.cdsOrProve(std.testing.allocator, "bsvz-zkp/cds-or/test/v1", G, &statements, 0, x0);
    defer zkp.sigma.cdsOrProofDeinit(proof, std.testing.allocator);
...
test "cds-or rejects length mismatch" {
    ...
    try std.testing.expect(!zkp.sigma.cdsOrVerify("bsvz-zkp/cds-or/test/v1", G, &empty, proof));
}
